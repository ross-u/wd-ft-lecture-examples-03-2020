...
function splitArr(booksMixed) {
  const authorsArr = [];
  const booksArr = [];

  // Split the mixed array into two arrays
  booksMixed.forEach((bookObj) => {
    authorsArr.push(bookObj.author);
    bookObj.author = undefined;
    booksArr.push(bookObj)
  })

  // return an array containing the filtered `authors` and `books` arrays
  return { authorsArr, booksArr } 
}
...
.then( (x) => {
  ...
  // 2.1 CREATE THE DOCUMENTS FROM THE ARRAY OF AUTHORS
  const { authorsArr } = splitArr(books);
  
  return Author.create(authorsArr);
})
.then( (createdAuthors) => {

  // Update the books array and add the author id to each book object
  const { booksArr } = splitArr(books);
  const updatedBooks = booksArr.map((book, index) => {
    book.author = createdAuthors[index]
    return book; 
  });

  // 2.2 CREATE THE DOCUMENTS FROM THE UPDATED ARRAY OF BOOKS

  return Book.create(updatedBooks);
})
.then((createdBooks) => {
